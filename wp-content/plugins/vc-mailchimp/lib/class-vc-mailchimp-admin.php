...
<?php
	...
	function initialize_vc_mailchimp_options() {
		...
		add_settings_field(
		 'clear_cache',
		 'Clear List Cache',
		 array( $this, 'vc_mailchimp_cache_callback' ),
		 'vc_mailchimp_settings',
		 'mailchimp_settings_section'
		);
	...
	function vc_mailchimp_cache_callback(){
		// Render the output
		echo '<input type="button" class="button" id="vc_mailchimp_clear_cache" name="" value="Clear Cache" /><span id="vc_mailchimp_cache_clearing" style="display:none;vertical-align:middle">&nbsp; &nbsp;Clearing....</span><span id="vc_mailchimp_cache_cleared" style="display:none;vertical-align:middle">&nbsp; &nbsp;<span style="color:green" class="dashicons dashicons-yes"></span>Cleared</span>';
		echo '<p class="description">Clear the List cache</p>';
		echo "<script>
		jQuery(function($) {
		$(document).on('click','#vc_mailchimp_clear_cache',function(){
			$('#vc_mailchimp_cache_cleared').hide();
         	$('#vc_mailchimp_cache_clearing').fadeIn();

         	$.post('".admin_url('admin-ajax.php')."', {action: 'vc_mailchimp_clear_cache'}, function(data, textStatus, xhr) {
         			$('#vc_mailchimp_cache_clearing').hide();
         			$('#vc_mailchimp_cache_cleared').fadeIn();
         	});
         })
		})
		</script>";
    ...
    */
	public function showVcVersionNotice() {
		...
		echo '
		<div class="updated">
		  <p>' . sprintf( __( '<strong>%s</strong> requires <strong><a href="https://codecanyon.net/item/visual-composer-page-builder-for-wordpress/242431?ref=webholics" target="_blank">Visual Composer</a></strong> plugin to be installed and activated on your site.', 'vc_extend' ), 'Visual Composer Mailchimp' ) . '</p>
		</div>';
	}
}
