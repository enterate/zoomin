<?php
/*
 * SiteOrigin Page Builder Customizations
		...
		if ( ! empty( $gutter ) ) {
			...
			if ( ! empty( $match[1] ) ) {
				...
				$css->add_cell_css( $post_id, $gi, false, '', array(
					'padding-left'  => $margin_half . '!important',
					'padding-right' => $margin_half . '!important',
				) );
...
function inbound_panels_row_style_fields( $fields ) {
	...
	$padding_top    = isset( $current['padding_top'] ) ? $current['padding_top'] : inbound_option( 'pagebuilder_row_padding_top', '0' ) . "px";
	$padding_right  = isset( $current['padding_right'] ) ? $current['padding_right'] : inbound_option( 'pagebuilder_row_padding_right', '0' ) . "px";
	$padding_bottom = isset( $current['padding_bottom'] ) ? $current['padding_bottom'] : inbound_option( 'pagebuilder_row_padding_bottom', '0' ) . "px";
	$padding_left   = isset( $current['padding_left'] ) ? $current['padding_left'] : inbound_option( 'pagebuilder_row_padding_left', '0' ) . "px";
...
function inbound_panels_widget_style_fields( $fields ) {
	...
	$current        = isset( $_REQUEST['style'] ) ? $_REQUEST['style'] : array();
	$padding_top    = isset( $current['padding_top'] ) ? $current['padding_top'] : "0px";
	$padding_right  = isset( $current['padding_right'] ) ? $current['padding_right'] : "0px";
	$padding_bottom = isset( $current['padding_bottom'] ) ? $current['padding_bottom'] : "0px";
	$padding_left   = isset( $current['padding_left'] ) ? $current['padding_left'] : "0px";
	...
	if ( isset ( $fields['padding'] ) ) {
		$fields['padding'] = array(
			'name'        => __( 'Padding', 'inbound' ),
			'type'        => 'measurement',
			'group'       => 'layout',
			'description' => esc_html__( 'Padding around the entire widget.', 'inbound' ),
			'priority'    => 7,
			'multiple'    => true,
			'default'     => $padding
		);
...
add_filter( 'siteorigin_panels_row_style_css', 'inbound_row_style_css', 11, 2 );
function inbound_row_style_css( $css, $style ) {
	/*
	 * Legacy Functions to apply padding
	 */
	...
	if ( empty ( $style['padding'] ) ) {
		...
		$css['padding'] = $pad_top . " " . $pad_right . " " . $pad_bottom . " " . $pad_left;
	}
...
add_filter( 'siteorigin_panels_widget_style_css', 'inbound_widget_style_css', 11, 2 );
function inbound_widget_style_css( $css, $style ) {
	/*
	 * Legacy Functions to implement widget style css
	 */
...
function inbound_panels_widget_style_attributes( $attr, $style ) {
	...
	if ( isset( $style['hide_on_mobile'] ) && $style['hide_on_mobile'] == 1 ) {
		$attr['class'][] = "hide-on-mobile";
	}
	if ( isset( $style['hide_on_tablet'] ) && $style['hide_on_tablet'] == 1 ) {
		$attr['class'][] = "hide-on-tablet";
	}
	if ( isset( $style['hide_on_desktop'] ) && $style['hide_on_desktop'] == 1 ) {
		$attr['class'][] = "hide-on-desktop";
	}
...
/*
 * Other Filters and Utility Functions
 */

function inbound_panels_get_widget_id() {
	global $inbound_last_widget_id;
	if ( empty( $inbound_last_widget_id ) ) {
		$inbound_last_widget_id = 0;
	}
...
function inbound_enqueue_panels_styles() {
	if ( function_exists( 'siteorigin_panels_render' ) && is_singular( 'page' ) && inbound_is_pagebuilder() ) {
		wp_enqueue_style( 'siteorigin-panels-front' );
	}
}
add_action( 'wp_enqueue_scripts', 'inbound_enqueue_panels_styles' );